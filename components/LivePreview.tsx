...
import { ArrowDownTrayIcon, PlusIcon, ViewColumnsIcon, DocumentIcon, CodeBracketIcon, XMarkIcon, ClipboardDocumentListIcon, CheckIcon } from '@heroicons/react/24/outline';
...
export const LivePreview: React.FC<LivePreviewProps> = ({ creation, isLoading, isFocused, onReset }) => {
    ...
    const handleCopyPrompt = () => {
        if (!creation) return;
        // Regex to extract the prompt from the hidden script tag
        ...
        if (match && match[1]) {
            ...
            // Fallback: Construct a basic prompt from the name/content if regex fails
            promptText = `Create a mobile app design for ${creation.name}. Include modern UI, Tailwind CSS, and full interactive flow.`;
        }
      ...
      <div className="bg-[#121214] px-4 py-3 flex items-center justify-between border-b border-zinc-800 shrink-0">
        ...
        <div className="flex items-center space-x-2 text-zinc-500">
            ...
            <span className="text-[11px] font-mono uppercase tracking-wider">
                {isLoading ? 'Architecting App System...' : creation ? creation.name : 'Preview Mode'}
            </span>
        </div>
            ...
            {!isLoading && creation && (
                <>
                    ...
                    <button 
                        ...
                        title="Copy Detailed Design Prompt"
                        className={`flex items-center space-x-2 px-3 py-1.5 rounded-md transition-all border ${copiedPrompt ? 'bg-green-500/10 border-green-500/50 text-green-400' : 'bg-zinc-800 border-transparent text-zinc-400 hover:text-white hover:bg-zinc-700'}`}
                    >
                        {copiedPrompt ? <CheckIcon className="w-4 h-4" /> : <ClipboardDocumentListIcon className="w-4 h-4" />}
                        <span className="text-xs font-medium hidden sm:inline">{copiedPrompt ? 'Copied!' : 'Copy Prompt'}</span>
                    </button>
      ...
      <div className="relative w-full flex-1 bg-[#09090b] flex overflow-hidden">
        {isLoading ? (
          <div className="absolute inset-0 flex flex-col items-center justify-center p-8 w-full">
             ...
             <div className="w-full max-w-md space-y-8">
                <div className="flex flex-col items-center">
                    ...
                    <h3 className="text-zinc-100 font-mono text-lg tracking-tight">Generating Design System</h3>
                    <p className="text-zinc-500 text-sm mt-2">Creating screens, flows, and component libraries...</p>
                </div>

                {/* Progress Bar */}
                <div className="w-full h-1 bg-zinc-800 rounded-full overflow-hidden">
                    <div className="h-full bg-blue-500 animate-[loading_4s_ease-in-out_infinite] w-1/3"></div>
                </div>

                 {/* Terminal Steps */}
                 <div className="border border-zinc-800 bg-black/50 rounded-lg p-4 space-y-3 font-mono text-sm">
                     <LoadingStep text="Analyzing user requirements" active={loadingStep === 0} completed={loadingStep > 0} />
                     <LoadingStep text="Drafting sitemap & flows" active={loadingStep === 1} completed={loadingStep > 1} />
                     <LoadingStep text="Designing UI components (Tailwind)" active={loadingStep === 2} completed={loadingStep > 2} />
                     <LoadingStep text="Compiling interactive prototype" active={loadingStep === 3} completed={loadingStep > 3} />
                 </div>
             </div>
          </div>
        ) : creation?.html ? (
          <>
            ...
            {/* App Preview Panel */}
            <div className={`relative h-full bg-zinc-900/50 flex items-center justify-center transition-all duration-500 ${showSplitView && creation.originalImage ? 'w-full md:w-1/2 h-1/2 md:h-full' : 'w-full'}`}>
                 {/* 
                    The generated HTML contains a mobile container. 
                    We render it in an iframe to isolate styles, but we want it to look like it's "floating" in the workspace.
                 */}
                 <iframe
